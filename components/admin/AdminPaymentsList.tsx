...
import { MessageSquare, Plus, X, Search, ExternalLink, FileText } from 'lucide-react'

type Tab = 'payments' | 'requests' | 'invoices' | 'logs'
...
export function AdminPaymentsList({ payments, requests, invoices, logs, users }: AdminPaymentsListProps) {
  ...
  return (
    <>
      <div className="admin-header">
        <div>
          <h1>Payments</h1>
          <p className="subtitle">Manage payments, invoices, requests, and view all transaction logs</p>
        </div>
      </div>

      <div style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
        {(['payments', 'requests', 'invoices', 'logs'] as const).map((t) => (
          <button
            key={t}
            ...
            onClick={() => setTab(t)}
          >
            {t === 'payments' ? `Payments (${payments.length})` : t === 'requests' ? `Requests (${requests.length})` : t === 'invoices' ? `Invoices (${invoices.length})` : `Logs (${logs.length})`}
          </button>
        ))}
      </div>
      ...
      {tab === 'invoices' && <InvoicesTab invoices={invoices} users={users} router={router} />}
...
function InvoicesTab({ invoices, users, router }: { invoices: Invoice[]; users: User[]; router: ReturnType<typeof useRouter> }) {
  const [showCreate, setShowCreate] = useState(false)
  const [formUserId, setFormUserId] = useState('')
  const [formAmount, setFormAmount] = useState('')
  const [formDescription, setFormDescription] = useState('')
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setCreating(true)
    try {
      await api.admin.invoices.create({
        userId: formUserId,
        amount: parseFloat(formAmount),
      ...
      showToast(e instanceof ApiError ? e.message : 'Failed to create invoice', 'error')
    }
    setCreating(false)
  }
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Create standalone invoices for users (not linked to a quotation).</p>
        <button className="btn btn-primary" style={{ display: 'flex', alignItems: 'center', gap: 6 }} onClick={() => setShowCreate(true)}>
      ...
      {invoices.length === 0 ? (
        <EmptyState title="No direct invoices" description="Create an invoice for a user to get started." />
      ) : (
        <div className="overflow-x-auto border border-[var(--border)] rounded-lg">
          <table className="admin-table">
            <thead>
            ...
            <tbody>
              {invoices.map((inv) => (
                <tr key={inv.id}>
                  <td><strong>{inv.user?.name || 'Unknown'}</strong><div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{inv.user?.email}</div></td>
                  <td><strong>{inv.invoiceNumber}</strong></td>
                  <td><strong>{formatCurrency(Number(inv.amount))}</strong></td>
                  <td style={{ fontSize: 13, maxWidth: 250, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{inv.description || '-'}</td>
                  <td><Badge variant={inv.status}>{inv.status}</Badge></td>
                  <td style={{ fontSize: 12 }}>{formatDateTime(inv.issuedAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="modal-overlay" onClick={() => setShowCreate(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ textAlign: 'left', maxWidth: 520, maxHeight: '90vh', overflow: 'auto' }}>
            ...
            <form onSubmit={handleCreate}>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                <button type="button" className="btn btn-secondary" onClick={() => setShowCreate(false)}>Cancel</button>
                <button type="submit" className="btn btn-primary" disabled={creating || !formUserId || !formAmount}>
